...
| 维度 | 定义 |
|:----|:-----|
| 步骤 | ①检测成功操作 → ②复盘成功原因 → ③提炼可复用模式 → ④写入 success_patterns → ⑤去伪存真验证效果 → ⑥有效：固化+举一反三泛化；无效：迭代或淘汰 |
...
| 维度 | 定义 |
|:----|:-----|
| 步骤 | ①收集 strike 和失败数据 → ②拆解问题模式 → ③对比期望行为：**锚定优先级**——优先检索 success_patterns 中同场景高置信度模式（≥4.0）；若无，则回退至"原始用户意图（intent_summary）"作为基线标准 → ④总结根因 → ⑤生成改进规则 → ⑥写入规则库 → 触发下一轮 |
...
| 约束 | 与攻七行动期严格隔离（守三运行时攻七不运行）；深度复盘期间不处理新任务 |
...
| 维度 | 定义 |
|:----|:-----|
| 步骤 | ①检测错误 → 立即自动修复（立改）→ 验证修复结果（改毕验）→ 修复成功则向攻七输出成功模式 ②同类错误再次出现 → 去伪存真归因过滤 → 内生惯性则写入硬阻断（dgen_override），外生变量则调整策略 ③阻断仍出现第3次 → 推翻原阻断方案，通知用户，系统升级 |
| 时机 | 同类错误发生时实时触发 |
...
| 维度 | 定义 |
|:----|:-----|
| 步骤 | ①收集本轮决策轨迹与自照报告 → ②审视执行模式：是否冗余重复？是否偏离初心？是否简洁高效？是否因主动冒险而获得了超额收益？如有，生成"勇气信号"，通过P6语义记忆给予正面加权，对冲纠偏偏好——勇气信号衰减采用半衰期式衰减：每轮剩余值 × 0.6（约每2轮减半），3轮后约剩10%，5轮后不足1%（注：定稿文字为约数，按×0.5估算；实现按公式每轮×0.6：3轮≈21.6%、5轮≈7.8%，以公式为准），自然归零 → ③生成方向校准信号 → ④归档至 Mindol codex → ⑤通过 P6 语义记忆静默影响下一轮裁决 |
| 时机 | 完形律封存本轮后 + 守三深度复盘结束后（自照镜跟随守三深度复盘的频率触发——默认每10轮或每日——而非每轮都照。完形律封存工作内存并归档至长期记忆，自照镜回望归档后的封存包，两者不冲突。守三深度复盘未触发时，自照镜静默跳过） |
