...
| 四 | 举一反三 | 通变门 | 一法通 → 三法生 → 百法衍 → 验而归真 | 跨域泛化：从单模式推导跨场景候选（语义距离<0.7）→ staging → 去伪存真回归校验 |
| 五 | 去伪存真 | 真伪门 | 言必有证 → 证必可验 → 验证为真 | 真伪验证：P0 无条件优先；假信息绝不进入权衡；暂存 50轮/7天 超时淘汰 |
| 六 | 预策 | 裁决律 | 汇而衡之 → 预而策之 → 决而行之 → 复而平之 | 权衡决断：汇→衡→预→策→行→复；P0-P6 硬优先级，秒级响应 |
