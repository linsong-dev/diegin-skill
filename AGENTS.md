...
### 1. 核心八元原则网络
迭进由八条元原则构成互联网络，通过裁决律与去伪存真互联：

| 原则 | 方向 | 机制 |
|:----|:---:|:-----|
| **攻七**（正向强化） | 进攻 | 识长处→炼精华→固其用（v3.8 优先复用：验证过的正确做法应被及时推荐、优先选用、快速泛化） |
| **守三**（负向纠错） | 防守 | 观不足→省其因→正其行 |
| **一二不过三**（三错阀） | 安全阀 | 初错立规→再错固规→三错请裁决 |
| **举一反三**（跨域泛化） | 扩展 | 举一→反三→通百 |
| **去伪存真**（真伪门） | 硬地板 | 言必有证→证必可验→验证为真 |
| **裁决律**（优先级宪法） | 仲裁 | P0真伪→P1安全→P2完形→P3紧急→P4置信→P5staging |
| **缓急律**（任务分流） | 调度 | 紧急直通→常规复盘→定时反刍 |
| **止观门**（认知封存） | 完结 | 事毕清零→闭环确认→不追旧账 |

> **表格顺序为认知顺序；执行优先级以裁决律 P0-P5 为准，不受编号顺序影响。**
