...
### 14. JSON BOM X    JSONXCodexXBOM

**问题**: hooks.json 被 PowerShell Set-Content 写入 UTF-8 BOM，Codex Rust JSON 解析器无法处理

|    |   |
|:---|:---|
|   ID | `rule_json_no_bom` |
| X | critical |
| X |   JSON X .NET XCodex/hooks/marketplace  |
|    | X PowerShell `Set-Content -Encoding UTF8`   JSONX Python `json.dump()` |
|    | 2026-07-10 hooks.json X |

### 15.    UI X   X + X

**X** X UI X      marketplace Xplugins X
...
| X | high |
| X | X UI X |
|    | X `~/.agents/plugins/marketplace.json`   plugins X**  **   `config.toml` X `[marketplaces.personal]` |
|    | 2026-07-10 X UI X |

### 16. Windows BOM X   X BOM

**X** 3 X .py X BOMX PowerShell X

|    |   |
|:---|:---|
|   ID | `rule_windows_bom_audit` |
| X | high |
| X | Windows X/   .py/.json/.ps1 X |
|    | X 3 X EF BB BFX |
|    | 2026-07-10 BOM X |
...
|    |   |
|:---|:---|
|   ID | `pat_use_plugin_creator_first_001`X |
| X | X/X |
| X | X plugin-creator X |
|    | 2026-07-10 |
...
|   ID | X | X | X |
|:---|:---:|:---|:---:|
| rule_word_meaning_confirm | high | X | X |
| rule_scope_full_check | high | X | X |
| rule_check_before_conclude | high | X | X |
| rule_extract_full_scope | high | X | X |
| rule_clean_verify_layered | critical | X3X | X |
| rule_delivery_full_audit | critical | X | X |
| rule_powershell_escape_triple_lock | critical | PowerShell X | X |
| rule_cmd_test_before_run | high | X | X |
| rule_toolchain_path_verify | high | X | X |
| rule_encoding_pre_check | high | X | X |
| rule_verify_command_exitcode | critical | X | X |
| rule_dry_run_before_batch | high | X dry-run | X |
| rule_tool_selection_fastest | medium | X | X |
| rule_json_no_bom | critical | JSON   Codex X BOM | 2026-07-10 |
| rule_plugin_ui_display_requirements | high |    UI   marketplace    + X | 2026-07-10 |
| rule_windows_bom_audit | high | X3   | 2026-07-10 |
| pat_use_plugin_creator_first_001 |    | X | 2026-07-10 |
...
导致 Python/JSON 解析器崩溃、AI 上下文注入乱码、hooks 中文变 X。
