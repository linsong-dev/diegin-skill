...
**2026-08-18/19（律令九章 v3.9.2）**

- **恒常门跨对话恢复破冰（恢复率 0 → 首次实证）**：2026-08-18 用户一句「恢复 A股模拟盘那个任务」→ 意图匹配唯一高置信命中（0.760，领先次名 0.286）→ 任务自动恢复并继续执行；v3.9.1 起支持无 task_id 的自然语言模糊恢复。
- **一二不过三 9 次阻断、同类错误不再复发**：累计 9 次阻断归因到 3 类根因，铁墙级规则（critical/blocking）直接拦在阻断层，不再走「重试 → 再错」循环。
- **死规则治理（去伪存真）**：215→202 条死规则被揪出，其中 7 条 BOM 编码规则全部失效、同一编码问题 4 天复发 4 次；触发验证门上线后不再复发。
- **推广数据包就绪**：64 条拦截规则（active 53 + critical 2 + blocking 1 + deprecating 7 + staging 1）/ 成功模式 21 条（平均置信度 4.95/5.0）/ Mindol 语义记忆 30,579 单元 / tests/evo 268 项全过 / test_all 32/32 全绿。

**2026-08-10（v3.8 时代，历史案例）**
